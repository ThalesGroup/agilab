...
def attach_all_subprojects(root_py: Optional[Path]) -> None:
    ...
    generate_core_run_configs(CORE_DIR)

    # optional: patch everything (apps + core) to enforce WORKING_DIRECTORY/env/module
    patch_run_configs_for([p.name for p in [APPS_DIR, CORE_DIR]])
...
def generate_core_run_configs(core_dir: Path):
    """
