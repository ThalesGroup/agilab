...
.. literalinclude:: snippets/agi_core_minimal_app_env.py
   :language: python

Cell 2: install the worker if needed, then run the visible ``AGI.run(...)`` call.

.. literalinclude:: snippets/agi_core_minimal_app_run.py
   :language: python

Cell 3: inspect the run artifacts.

.. literalinclude:: snippets/agi_core_minimal_app_log_root.py
   :language: python

How this maps back to the web UI
--------------------------------

- Notebook ``notebook_app_env(\"minimal_app_project\")`` is the compact form of
  ``AgiEnv(...)`` and corresponds to choosing the built-in Minimal App example app
  in **PROJECT**.
...
- Notebook ``AGI.run(...)`` corresponds to the generated snippet from
  **ORCHESTRATE**.
- The output path under ``~/log/execute/minimal_app`` is the same family of artifacts
