...
- `tools/global_pipeline_runner_state_report.py --compact` projects the plan
  ...
  apps
- the PIPELINE page now includes an expanded `Run apps in order` surface that
  can choose a plan source, edit steps, created outputs, and used outputs
  through selector-driven workspace drafts and read-only summaries, validate
  the plan without hand-editing docs files, reset the persisted preview state,
  show readiness KPIs, optional graph and output details, and preview the next
  ready step without claiming live app execution
