...
**Core services**
    ...
    - Apps under ``src/agilab/apps`` stay isolated but consume the same base
      ...
      such as ``mycode_project`` and ``flight_project``; additional templates can
...
.. literalinclude:: directory-structure.txt
   :language: none
   :caption: Generated by ``docs/gen-docs.py`` (gitignore-aware)

Refresh the tracked tree after file changes with ``uv run --group sphinx --dev docs/gen-docs.py --refresh-generated``.
