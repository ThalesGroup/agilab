...
def print_emoticon(result, success_check=None):
    ...
    if success_check(result):
        print(f"{GREEN}✅ mycode is working{RESET}")
    else:
        print(f"{RED}❌ mycode fail to run{RESET}")
...
def main():
    ...
    tests = [
        ("uv -q run test/test_mycode_manager.py", "mycode_manager"),
        ("uv -q run test/test_mycode_worker.py", "mycode_worker"),
    ]
