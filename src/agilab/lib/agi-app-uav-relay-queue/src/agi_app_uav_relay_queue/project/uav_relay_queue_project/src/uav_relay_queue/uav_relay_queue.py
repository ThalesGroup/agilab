...
from .app_args import ArgsOverrides, UavRelayQueueArgs, dump_args, ensure_defaults, load_args, merge_args

logger = logging.getLogger(__name__)


class UavRelayQueue(BaseWorker):
    ...
    def __init__(
        self,
        env,
        args: UavRelayQueueArgs | None = None,
        **kwargs: ArgsOverrides,
        ...
        if args is None:
            try:
                args = UavRelayQueueArgs(**kwargs)
            except ValidationError as exc:
                raise ValueError(f"Invalid UavRelayQueue arguments: {exc}") from exc
        ...
        try:
            ...
            raise ValueError(f"Invalid UavRelayQueue data_in/data_out path: {exc}") from exc
...
class UavQueue(UavRelayQueue):
    """Compatibility alias preserving the historical lightweight UAV queue manager name."""
...
__all__ = ["UavRelayQueue", "UavRelayQueueApp", "UavQueue", "UavQueueApp"]
