...
class _FallbackPlotlyLayout(dict):
...
class _FallbackPlotlyFigure(dict):
    ...
    @property
    def layout(self) -> _FallbackPlotlyLayout:
        layout = self.setdefault("layout", {})
        if isinstance(layout, _FallbackPlotlyLayout):
            return layout
        normalized = _FallbackPlotlyLayout(layout)
        self["layout"] = normalized
        return normalized

    def update_layout(self, **kwargs: object) -> "_FallbackPlotlyFigure":
        self.layout.update(kwargs)
        return self


def _plotly_unavailable_figure(kind: str, trace_count: int = 0, **layout: object) -> _FallbackPlotlyFigure:
