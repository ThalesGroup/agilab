...
def _decision_figure(
    ...
    *,
    ...
    if go is None:
        ...
        return _plotly_unavailable_figure(
            "Decision boundary", trace_count, height=height, showlegend=show_legend
...
def _history_figure(history: pd.DataFrame, *, height: int = 280, show_legend: bool = True) -> go.Figure:
    if go is None:
        return _plotly_unavailable_figure(
            "Training history", 4 if not history.empty else 0, height=height, showlegend=show_legend
        )  # type: ignore[return-value]
    figure = go.Figure()
...
def _activation_figure(activation_maps: pd.DataFrame, layer: int, neuron: int) -> go.Figure:
    if go is None:
        ...
        return _plotly_unavailable_figure(
            "Activation map", 1 if not selected.empty else 0, height=420
        )  # type: ignore[return-value]
    figure = go.Figure()
    selected = activation_maps[(activation_maps["layer"] == layer) & (activation_maps["neuron"] == neuron)]
...
def _network_figure(layers: pd.DataFrame) -> go.Figure:
    if go is None:
        return _plotly_unavailable_figure(
            "Network diagnostics", 2 if not layers.empty else 0, height=260
        )  # type: ignore[return-value]
    figure = go.Figure()
    if not layers.empty:
...
def _loss_landscape_figure(landscape: pd.DataFrame) -> go.Figure:
    if go is None:
        return _plotly_unavailable_figure(
            "Loss landscape", 3 if not landscape.empty else 0, height=460
        )  # type: ignore[return-value]
    figure = go.Figure()
