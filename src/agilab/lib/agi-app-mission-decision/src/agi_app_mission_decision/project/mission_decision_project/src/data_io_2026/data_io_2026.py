"""Manager for the public Mission Decision built-in demo."""
...
class DataIo2026(BaseWorker):
    ...
    def _ensure_dataset(self, data_in: Path) -> None:
        ...
        if not sample.is_file():
            raise FileNotFoundError(f"Bundled Mission Decision scenario missing: {sample}")
        destination = data_in / sample.name
        shutil.copy2(sample, destination)
        logger.info("Seeded Mission Decision sample scenario at %s", destination)
