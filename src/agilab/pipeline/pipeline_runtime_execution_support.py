...
from agilab.diagnostics.runtime_diagnostics import coerce_diagnostics_verbose
...
def ensure_safe_service_template(
    ...
    *,
    ...
    template_path = stages_file.parent / template_filename
...
def run_locked_stage(
    ...
    *,
    ...
    label_for_stage_runtime_fn: Callable[[str | Path | None], str] | Callable[..., str],
    ...
    python_for_stage_fn: Callable[..., Path],
    ...
    if stored_placeholder is not None:
        stored_placeholder.caption("Starting stage execution…")
    ...
    try:
        selected_map_entry = normalize_runtime_path(selected_map.get(stage, ""))
        ...
        log_file_path, log_error = prepare_run_log_file(index_page_str, env, f"stage_{stage + 1}")
        if log_file_path:
            push_run_log(
                index_page_str,
        ...
        try:
            ...
            env_label = label_for_stage_runtime_fn(venv_root, engine=engine, code=code_to_run)
            summary = stage_summary({"Q": entry.get("Q", ""), "C": code_to_run}, 60)
            stage_tags = {
            ...
            with start_mlflow_run_fn(
                ...
                stage_env = build_mlflow_process_env_fn(
                    env,
                ...
                if engine == "runpy":
                    ...
                    python_cmd = python_for_stage_fn(venv_root, engine=engine, code=code_to_run)
                ...
                if stage_tracking:
                    log_mlflow_artifacts_fn(
                        stage_tracking,
                        text_artifacts={f"stage_{stage + 1}/stdout.txt": preview or ""},
