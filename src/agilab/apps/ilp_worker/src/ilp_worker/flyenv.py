...
#        self.edges_dict = None
    ...
    def generate_environment(self, topology, listofdemands):
        graph = generate_nx_graph(topology)
        self._initialise_graph(graph, listofdemands)

    def generate_environment_from_graph(self, graph, listofdemands):
        """Load an explicitly provided topology (typically from LinkSim datasets)."""
        self._initialise_graph(graph, listofdemands)

    def _initialise_graph(self, graph, listofdemands):
        # The nx graph will only be used to convert graph from edges to nodes
        self.graph = graph.copy()
        self.nodes = list(self.graph.nodes)
        if listofdemands:
            self.listofDemands = list(listofdemands)
        else:
            self.listofDemands = DEMAND_DEFAULT[:]
        self.max_demand = max(self.listofDemands) if self.listofDemands else 1
