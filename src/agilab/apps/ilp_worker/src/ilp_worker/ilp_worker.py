    ...
    def work_pool(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if data_out:
            try:
                ...
                out_dir = Path(data_out).expanduser()
                out_dir.mkdir(parents=True, exist_ok=True)
                outfile = out_dir / f"allocations_batch_{batch_id}.json"
                outfile.write_text(json.dumps(allocations, indent=2), encoding="utf-8")
            except Exception as exc:  # pragma: no cover - defensive logging
                import logging

                logging.warning("Failed to persist ILP allocations: %s", exc)
