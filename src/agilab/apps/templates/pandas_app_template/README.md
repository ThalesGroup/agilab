...
distributed Pandas-style data pipeline.

## What This Template Provides

- A minimal app manager package under `src/pandas_app/`.
- Pydantic-backed runtime arguments in `pandas_app_args.py`.
...
2. Rename `your_pandas_project` in `pyproject.toml`.
3. Rename the Python package from `pandas_app` to the app package name.
4. Update `src/app_settings.toml` so `[args].data_in` points at the app dataset.
5. Replace the template hooks with the real Pandas pipeline and output contract.
