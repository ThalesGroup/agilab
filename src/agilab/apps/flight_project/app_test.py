...
async def main() -> None:
    ...
    wenv = env.wenv_abs
    for cmd in [
        f"uv --preview-features extra-build-dependencies run --no-sync --project \"{wenv}\" python -m agi_node.agi_dispatcher.build --app-path \"{wenv}\" "
        f"-q bdist_egg --packages agi_dispatcher,polars_worker -d \"{wenv}\"",
        f"uv --preview-features extra-build-dependencies run --no-sync --project \"{wenv}\" python -m agi_node.agi_dispatcher.build --app-path \"{wenv}\" "
        f"-q build_ext -b \"{wenv}\"",
        f"uv --preview-features extra-build-dependencies run --no-sync --project \"{script_dir}\" \"{script_dir}/test/_test_{target_name}_manager.py\"",
        f"uv --preview-features extra-build-dependencies run --no-sync --project \"{worker_repo}\" \"{script_dir}/test/_test_{target_name}_worker.py\"",
    ]:
