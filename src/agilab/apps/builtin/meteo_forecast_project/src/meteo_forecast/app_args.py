"""Argument helpers for the Meteo forecast builtin app."""
...
class MeteoForecastArgs(BaseModel):
    ...
    data_in: Path = Field(default_factory=lambda: Path("meteo_forecast/dataset"))
    data_out: Path = Field(default_factory=lambda: Path("meteo_forecast/results"))
    files: str = "*.csv"
