...
PAGE_ID = "meteo_forecast_project:app_args_form"
...
def _load_current_args(settings_path: Path) -> MeteoForecastArgs:
    try:
        ...
        st.warning(f"Unable to load Meteo Forecast args from `{settings_path}`: {exc}")
...
try:
    ...
    st.error("Invalid Meteo Forecast parameters:")
