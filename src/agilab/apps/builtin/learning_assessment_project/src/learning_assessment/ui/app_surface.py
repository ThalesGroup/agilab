...
from learning_assessment.curriculum import (  # noqa: E402
    build_math_program_2026_coverage_report,
)
from learning_assessment.classroom import (  # noqa: E402
...
def _load_runtime_env_and_args(active_app_path: Path):
    ...
    from learning_assessment import app_args
    ...
    *,
    ...
    if runtime_env is not None:
        ...
        for target in (
            ...
            if target:
                _append_unique_path(
                    paths, export_root / target / "learning_assessment" / "classroom"
                )

        data_out = Path(
            str(getattr(runtime_args, "data_out", "learning_assessment/reports"))
        )
    ...
    if active_app_path is not None:
        for target in (
            ...
            _append_unique_path(
                paths,
                Path.home() / "export" / target / "learning_assessment" / "classroom",
            )
        _append_unique_path(
            paths, active_app_path / "learning_assessment" / "reports" / "classroom"
        )
    return paths
    ...
    with classroom_tab:
        ...
        with st.expander("Submission inbox", expanded=False):
            if classroom_inbox_dir is None:
                st.caption(
                    "Open Learning & Assessment from an active project to enable classroom JSON intake."
                )
            else:
                st.caption(
