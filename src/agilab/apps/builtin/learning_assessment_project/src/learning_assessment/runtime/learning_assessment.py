...
class LearningAssessment(BaseWorker):
    ...
    def __init__(
        self,
        env,
        args: LearningAssessmentArgs | None = None,
        ...
        **kwargs: ArgsOverrides,
        ...
        if args is None:
            try:
                args = LearningAssessmentArgs(**kwargs)
        ...
        if self.args.reset_target:
            reset_path = self._safe_share_reset_path(
                env,
                self.data_out,
                protected_paths=(self.args.data_in, self.args.submission_inbox),
                label="data_out",
            )
    ...
    def analysis_artifact_dir(self) -> Path:
        ...
        return export_root / self.env.target / "learning_assessment"
...
class LearningAssessmentApp(LearningAssessment):
    """Compatibility alias retaining the descriptive *App suffix."""


__all__ = ["LearningAssessment", "LearningAssessmentApp"]
