...
from ..domain.generator import DEFAULT_GPT_OSS_ENDPOINT, DEFAULT_GPT_OSS_MODEL


class LearningAssessmentArgs(BaseModel):
    """Runtime parameters for the TeSciA diagnostic application."""

    model_config = ConfigDict(extra="forbid")

    data_in: Path = Field(default_factory=lambda: Path("learning_assessment/cases"))
    data_out: Path = Field(default_factory=lambda: Path("learning_assessment/reports"))
    submission_inbox: Path = Field(
        default_factory=lambda: Path("learning_assessment/submissions")
    )
    ...
    def _validate_file_glob(self) -> "LearningAssessmentArgs":
...
class LearningAssessmentArgsTD(TypedDict, total=False):
    data_in: str
    data_out: str
...
ArgsModel = LearningAssessmentArgs
ArgsOverrides = LearningAssessmentArgsTD


def load_args(
    settings_path: str | Path, *, section: str = "args"
) -> LearningAssessmentArgs:
    return load_model_from_toml(LearningAssessmentArgs, settings_path, section=section)


def merge_args(
    base: LearningAssessmentArgs,
    overrides: LearningAssessmentArgsTD | None = None,
) -> LearningAssessmentArgs:
...
def ensure_defaults(args: LearningAssessmentArgs, **_: Any) -> LearningAssessmentArgs:
    return args
