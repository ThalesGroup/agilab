...
from learning_assessment import (  # noqa: E402 - requires the project source path above
...
PAGE_ID = "learning_assessment_project:app_args_form"
...
artifact_target = str(
    getattr(env, "target", "") or getattr(env, "app", "") or "learning_assessment_project"
)
export_root = Path(getattr(env, "AGILAB_EXPORT_ABS", Path.home() / "export")).resolve(
    strict=False
)
artifact_target = Path(artifact_target).name
artifact_root = export_root / artifact_target / "learning_assessment"
...
_seed(
    "submission_inbox",
    str(current_payload.get("submission_inbox", "learning_assessment/submissions") or ""),
)
...
try:
    validated = LearningAssessmentArgs(**candidate)
