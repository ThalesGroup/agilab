"""Reduce-contract adoption for the built-in Weather forecast app."""
...
REDUCE_ARTIFACT_NAME = "weather_forecast_reduce_summary"
REDUCER_NAME = "weather_forecast.forecast-metrics.v1"
...
def _validate_meteo_forecast_artifact(artifact: ReduceArtifact) -> None:
    ...
    if int(payload["backtest_rows"]) <= 0:
        raise ValueError("weather_forecast reducer produced no backtest rows")
...
def write_reduce_artifact(
    ...
    *,
    ...
    partial = partial_from_forecast_metrics(
        metrics,
        partial_id=f"weather_forecast_worker_{worker_id}",
