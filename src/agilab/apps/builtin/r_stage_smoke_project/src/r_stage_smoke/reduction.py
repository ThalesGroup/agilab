"""Reduce-contract adoption for R stage smoke evidence."""
...
REDUCE_ARTIFACT_NAME = "r_stage_smoke_reduce_summary"
REDUCER_NAME = "r_stage_smoke.json-artifact.v1"
...
def _metrics_from_summary(summary: Mapping[str, Any]) -> Mapping[str, Any]:
    ...
    if not isinstance(metrics, Mapping):
        raise ValueError("R stage smoke summary missing metrics")
    return metrics


def _merge_r_stage_partials(partials: Sequence[ReducePartial]) -> dict[str, Any]:
    run_count = 0
    n_sum = 0
    mean_sum = 0.0
    sd_sum = 0.0
...
def _validate_r_stage_artifact(artifact: ReduceArtifact) -> None:
    if int(artifact.payload["run_count"]) <= 0:
        raise ValueError("R stage smoke reducer produced no runs")
    if int(artifact.payload["n_sum"]) <= 0:
        raise ValueError("R stage smoke reducer produced no input observations")


R_STAGE_SMOKE_REDUCE_CONTRACT = ReduceContract(
    name=REDUCER_NAME,
    artifact_name=REDUCE_ARTIFACT_NAME,
    merge=_merge_r_stage_partials,
    validate_partial=require_payload_keys(*_REQUIRED_PAYLOAD_KEYS),
    validate_artifact=_validate_r_stage_artifact,
    metadata={
...
def partial_from_r_stage_summary(
...
def build_reduce_artifact(partials: Sequence[ReducePartial]) -> ReduceArtifact:
    return R_STAGE_SMOKE_REDUCE_CONTRACT.build_artifact(partials)
    ...
    *,
    ...
    partials = [
        partial_from_r_stage_summary(
            summary,
            partial_id=f"r_stage_smoke_worker_{worker_id}_{index}",
