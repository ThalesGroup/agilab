    ...
    if st.session_state.get("toggle_edit", True):
        ...
        with c2:
            data_in = st.text_input(
                ...
                key=f"data_in",
                help=f"Manager workers read from {env.agi_share_path_abs}/<your path> when running locally.",
            )
        ...
        with c3:
            data_out = st.text_input(
                ...
                key=f"data_out",
                help=f"Outputs will be written under {env.agi_share_path_abs}/<your path>.",
            )
        ...
        with c8:
            datemin = st.date_input("from Date", value=defaults_model.datemin, key=f"datemin")

        with c9:
            datemax = st.date_input("to Date", value=defaults_model.datemax, key=f"datemax")
        ...
        if data_source == "file":
            directory = env.agi_share_path_abs / data_in
