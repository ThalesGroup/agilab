"""Manager for the built-in scikit-learn pipeline app."""
...
class SklearnPipeline(BaseWorker):
    """Manager that dispatches one deterministic sklearn proof run."""
    ...
    def __init__(
        self,
        env,
        args: SklearnPipelineArgs | None = None,
        **kwargs: ArgsOverrides,
        ...
        try:
            ...
            raise ValueError(f"Invalid SklearnPipeline data_out path: {exc}") from exc
    ...
    def analysis_artifact_dir(self) -> Path:
        ...
        target = str(
            ...
            or "sklearn_pipeline_project"
        )
        return export_root / target / "sklearn_pipeline"
    ...
    def build_distribution(self, workers):
        try:
            ...
            worker_count = 1
        work_plan = [[["sklearn_pipeline"]]] + [[] for _ in range(worker_count - 1)]
        metadata = [[{"run": "sklearn_pipeline", "work_items": 1}]] + [[] for _ in range(worker_count - 1)]
        return work_plan, metadata, "run", "work_items", "items"


class SklearnPipelineApp(SklearnPipeline):
    """Compatibility class with the descriptive *App suffix."""


__all__ = ["SklearnPipeline", "SklearnPipelineApp"]
