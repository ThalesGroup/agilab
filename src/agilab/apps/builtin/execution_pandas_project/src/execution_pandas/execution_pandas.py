"""Execution playground manager for the pandas worker path."""
...
class ExecutionPandas(BaseWorker):
    """AGILab manager that generates a deterministic execution benchmark workload."""
    ...
    def __init__(
        self,
        env,
        args: ExecutionPandasArgs | None = None,
        **kwargs: ArgsOverrides,
        ...
        if args is None:
            try:
                args = ExecutionPandasArgs(**kwargs)
            except ValidationError as exc:
                raise ValueError(f"Invalid ExecutionPandas arguments: {exc}") from exc
        ...
        try:
            ...
            raise ValueError(f"Invalid ExecutionPandas data_in/data_out path: {exc}") from exc
...
class ExecutionPandasApp(ExecutionPandas):
    """Compatibility alias retaining the historic *App suffix."""


__all__ = ["ExecutionPandas", "ExecutionPandasApp"]
