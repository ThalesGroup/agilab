"""Manager for the built-in R runtime bridge app."""
...
class RRuntimeBridge(BaseWorker):
    ...
    def __init__(
        self,
        env,
        args: RRuntimeBridgeArgs | None = None,
        **kwargs: ArgsOverrides,
        ...
        if args is None:
            try:
                args = RRuntimeBridgeArgs(**kwargs)
    ...
    def analysis_artifact_dir(self) -> Path:
        ...
        target = str(
            ...
            or "r_runtime_bridge_project"
        )
        return export_root / target / "r_runtime_bridge"
    ...
    def build_distribution(self, workers):
        try:
            ...
            worker_count = 1
        work_plan = [[["r_runtime_bridge"]]] + [[] for _ in range(worker_count - 1)]
        metadata = [[{"run": "r_runtime_bridge", "work_items": 1}]] + [
            [] for _ in range(worker_count - 1)
        ]
        return work_plan, metadata, "run", "work_items", "items"


class RRuntimeBridgeApp(RRuntimeBridge):
    """Compatibility class with the descriptive *App suffix."""


__all__ = ["RRuntimeBridge", "RRuntimeBridgeApp"]
