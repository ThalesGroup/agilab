"""Worker for the built-in R runtime bridge app."""
...
def _args_with_defaults(value: Any) -> RRuntimeBridgeArgs:
    if isinstance(value, RRuntimeBridgeArgs):
        return value
...
class RRuntimeBridgeWorker(PandasWorker):
    ...
    def _current_args(self) -> RRuntimeBridgeArgs:
    ...
    def work_pool(self, _work_item):
        ...
        logger.info("wrote R runtime bridge evidence to %s", self.data_out)
