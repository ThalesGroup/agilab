"""Reduce-contract adoption for the built-in UAV queue app."""
...
REDUCE_ARTIFACT_NAME = "uav_queue_reduce_summary"
REDUCER_NAME = "uav_queue.queue-metrics.v1"
...
def _merge_uav_queue_partials(partials: Sequence[ReducePartial]) -> dict[str, Any]:
...
def _validate_uav_queue_artifact(artifact: ReduceArtifact) -> None:
    payload = artifact.payload
    if int(payload["scenario_count"]) <= 0:
        raise ValueError("uav_queue reducer produced no scenarios")
    if int(payload["packets_generated"]) <= 0:
        raise ValueError("uav_queue reducer produced no generated packets")


UAV_QUEUE_REDUCE_CONTRACT = ReduceContract(
    name=REDUCER_NAME,
    artifact_name=REDUCE_ARTIFACT_NAME,
    merge=_merge_uav_queue_partials,
    validate_partial=require_payload_keys(*_REQUIRED_PAYLOAD_KEYS),
    validate_artifact=_validate_uav_queue_artifact,
    metadata={
...
def partial_from_summary_metrics(
    ...
    *,
    ...
    if missing:
        ...
        raise ValueError(f"uav_queue summary metrics missing columns: {missing_text}")
...
def build_reduce_artifact(partials: Sequence[ReducePartial]) -> ReduceArtifact:
    return UAV_QUEUE_REDUCE_CONTRACT.build_artifact(partials)
...
def write_reduce_artifact(
    ...
    *,
    ...
    partial = partial_from_summary_metrics(
        metrics,
        partial_id=f"uav_queue_worker_{worker_id}_{metrics['artifact_stem']}",
