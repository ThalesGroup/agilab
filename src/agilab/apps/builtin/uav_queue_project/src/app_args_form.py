...
from uav_queue import UavQueueArgs, dump_args, load_args


PAGE_ID = "uav_queue_project:app_args_form"
...
def _load_current_args(settings_path: Path) -> UavQueueArgs:
    try:
...
st.caption(
    "This built-in app runs a lightweight UAV routing and queueing simulation with explicit "
...
try:
    ...
    if validated_payload != current_payload:
        ...
        app_settings["pages"]["view_module"] = ["view_uav_queue_analysis", "view_maps_network"]
