...
from weather_forecast_legacy import WeatherForecastLegacy, WeatherForecastLegacyArgs
from weather_forecast_legacy.reduction import (
...
def test_weather_forecast_legacy_manager_seeds_dataset_and_distribution(tmp_path: Path) -> None:
    ...
    assert manager.analysis_artifact_dir == env.AGILAB_EXPORT_ABS / "weather_forecast_legacy" / "forecast_analysis"
...
def test_weather_forecast_legacy_worker_exports_analysis_artifacts(tmp_path: Path) -> None:
    env = _make_env(tmp_path)
    args = WeatherForecastLegacyArgs(reset_target=True)
    manager = WeatherForecastLegacy(env, args=args)
...
def test_weather_forecast_legacy_worker_uses_share_path_without_export_attr(tmp_path: Path) -> None:
    env = _make_env(tmp_path)
    args = WeatherForecastLegacyArgs(reset_target=True)
    manager = WeatherForecastLegacy(env, args=args)
...
def test_weather_forecast_legacy_reduce_contract_merges_forecast_partials() -> None:
    base_metrics = {
