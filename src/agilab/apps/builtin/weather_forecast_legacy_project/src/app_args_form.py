...
from weather_forecast_legacy import WeatherForecastLegacyArgs, dump_args, load_args


PAGE_ID = "weather_forecast_legacy_project:app_args_form"
...
def _load_current_args(settings_path: Path) -> WeatherForecastLegacyArgs:
    try:
        return load_args(settings_path)
    except Exception as exc:
        st.warning(f"Unable to load Weather Forecast Legacy args from `{settings_path}`; restoring defaults.")
        st.code(str(exc))
        return WeatherForecastLegacyArgs()
...
try:
    validated = WeatherForecastLegacyArgs(**candidate)
except ValidationError as exc:
    st.error("Invalid Weather Forecast Legacy parameters:")
