"""Reduce-contract adoption for the built-in weather forecast legacy app."""
...
def _merge_weather_forecast_legacy_partials(partials: Sequence[ReducePartial]) -> dict[str, Any]:
...
def _validate_weather_forecast_legacy_artifact(artifact: ReduceArtifact) -> None:
    payload = artifact.payload
    if int(payload["forecast_run_count"]) <= 0:
        raise ValueError("weather_forecast_legacy reducer produced no forecast runs")
    if int(payload["backtest_rows"]) <= 0:
        raise ValueError("weather_forecast_legacy reducer produced no backtest rows")
    if not payload["stations"]:
        raise ValueError("weather_forecast_legacy reducer produced no station metadata")


WEATHER_FORECAST_LEGACY_REDUCE_CONTRACT = ReduceContract(
    name=REDUCER_NAME,
    artifact_name=REDUCE_ARTIFACT_NAME,
    merge=_merge_weather_forecast_legacy_partials,
    validate_partial=require_payload_keys(*_REQUIRED_PAYLOAD_KEYS),
    validate_artifact=_validate_weather_forecast_legacy_artifact,
    metadata={
...
def partial_from_forecast_metrics(
    ...
    *,
    ...
    if missing:
        ...
        raise ValueError(f"weather_forecast_legacy metrics missing columns: {missing_text}")
