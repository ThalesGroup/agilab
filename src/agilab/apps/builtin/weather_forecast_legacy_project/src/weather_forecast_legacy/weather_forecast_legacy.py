"""Manager for the built-in weather forecast legacy project."""
...
class WeatherForecastLegacy(BaseWorker):
        ...
        **kwargs: ArgsOverrides,
        ...
        if args is None:
            try:
                ...
                raise ValueError(f"Invalid WeatherForecastLegacy arguments: {exc}") from exc
        ...
        try:
            ...
            raise ValueError(f"Invalid WeatherForecastLegacy data_in/data_out path: {exc}") from exc
    ...
    def analysis_artifact_dir(self) -> Path:
        ...
        return export_root / self.env.target / "forecast_analysis"

    def _sample_dataset_source(self) -> Path:
        return Path(__file__).resolve().parent / "sample_data" / "meteo_fr_daily_sample.csv"
    ...
    def _ensure_dataset(self, data_in: Path) -> None:
        ...
        if not sample.is_file():
            raise FileNotFoundError(f"Bundled sample dataset missing: {sample}")
        destination = data_in / sample.name
        shutil.copy2(sample, destination)
        logger.info("Seeded weather forecast legacy sample dataset at %s", destination)
    ...
    def build_distribution(self, workers):
        ...
        if not files:
            raise FileNotFoundError(
                f"No weather CSV file found in {self.args.data_in} with pattern {self.args.files!r}"
            )
        ...
        for chunk in worker_chunks:
            ...
            metadata.append([{"file": batch_label, "size_kb": total_size_kb}])
