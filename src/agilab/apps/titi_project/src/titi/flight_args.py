"""Shared validation and persistence helpers for Titi project arguments."""
...
class TitiArgs(BaseModel):
    """Validated configuration for the Titi worker."""
    model_config = ConfigDict(extra='forbid', validate_assignment=True)
    data_source: Literal['file', 'hawk'] = 'file'
    data_uri: Path = Field(default_factory=lambda : Path('data/titi/dataset'))
    files: str = '*'
    ...
    def _check_datemin(cls, value: date) ->date:
        if value < _DATEMIN_LOWER_BOUND:
            raise ValueError(
                f'datemin must be on or after {_DATEMIN_LOWER_BOUND.isoformat()}'
                )
        return value
    ...
    def _check_datemax(cls, value: date, info: Any) ->date:
        ...
        if value > _DATEMAX_UPPER_BOUND:
            raise ValueError(
                f'datemax must be on or before {_DATEMAX_UPPER_BOUND.isoformat()}'
                )
        return value
    ...
    def _check_regex(cls, value: str) ->str:
        ...
        try:
            ...
            raise ValueError(
                f"The provided string '{value}' is not a valid regex."
                ) from exc
        return value
...
def load_args_from_toml(settings_path: (str | Path), section: str=ARGS_SECTION
    ) ->TitiArgs:
    """Load arguments from a TOML file, applying model defaults when missing."""
    return load_model_from_toml(TitiArgs, settings_path, section=section)


def merge_args(base: TitiArgs, overrides: (FlightArgsTD | None)=None
    ) ->TitiArgs:
    """Return a new instance with overrides applied on top of ``base``."""
    return merge_model_data(base, overrides)


def apply_source_defaults(args: TitiArgs, *, host_ip: (str | None)=None
    ) ->TitiArgs:
    ...
    if args.data_source == 'file':
        if not str(args.data_uri).strip():
            overrides['data_uri'] = 'data/titi/dataset'
        if not args.files:
            overrides['files'] = '*'
    else:
        if host_ip:
        ...
        if not current_uri.strip() or current_uri == 'data/titi/dataset':
...
def dump_args_to_toml(args: TitiArgs, settings_path: (str | Path), section:
    str=ARGS_SECTION, create_missing: bool=True) ->None:
    ...
    dump_model_to_toml(args, settings_path=settings_path, section=section,
