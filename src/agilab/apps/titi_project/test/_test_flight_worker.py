...
async def main():
    args = {'data_source': 'file', 'data_uri': 'data/titi/dataset', 'files':
        'csv/*', 'nfile': 1, 'nskip': 0, 'nread': 0, 'sampling_rate': 10.0,
        'datemin': '2020-01-01', 'datemax': '2021-01-01', 'output_format':
        'csv'}
    active_app = base_path.parents[1]
    sys.path.insert(0, active_app / 'src')
    sys.path.insert(0, str(Path.home() / 'wenv/titi_worker/dist'))
    ...
    menv = env.wenv_abs
    cmd = (
        f"uv run --project {menv} python {build} bdist_egg --packages 'base_worker, polars_worker' -d '{menv}'"
        )
