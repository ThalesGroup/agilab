...
def generate_nx_graph(topology):
    ...
    if topology == 0:
        ...
        graph = create_geant2_graph()
    elif topology == 2:
        graph = create_small_top()
    elif topology == 3:
...
#        G.get_edge_data(i, j)['bw_allocated'] = 0
        Id = Id + 1
 #   if topology != 3:
 #       nx.set_edge_attributes(graph, 10000, 'capacity')
 #       nx.set_edge_attributes(graph, 'sat', 'bearer')
