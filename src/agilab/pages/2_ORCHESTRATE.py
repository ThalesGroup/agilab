        ...
        _orchestrate_notebook_cell(
            ...
            "\n".join(
                [
                    ...
                    "",
                    "Notebook import remains on the PIPELINE page because import modifies pipeline steps.",
                ]
            ),
        )
    ]
...
def _render_orchestrate_notebook_expander(env: Any) -> None:
    ...
    with st.expander("Notebook", expanded=False):
        st.caption(
            ...
            "Import stays in PIPELINE because it changes pipeline step definitions."
        )
        if not snippets:
