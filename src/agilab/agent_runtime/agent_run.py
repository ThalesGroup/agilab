...
from agilab.agent_runtime.verification import (
    TRACE_KIND as TRACE_KIND,
    MANIFEST_FILENAME as MANIFEST_FILENAME,
    AgentRunSummary as AgentRunSummary,
    load_agent_run_manifest as load_agent_run_manifest,
    summarize_agent_run as summarize_agent_run,
    render_validation_markdown as render_validation_markdown,
    _path_from_artifact as _path_from_artifact,
    _summary_payload,
    validate_agent_run as _verify_agent_run,
)
...
def validate_agent_run(manifest_or_path: dict[str, object] | Path | str) -> dict[str, object]:
    """Verify evidence through the read-only owner, retaining public trace seams."""
    return _verify_agent_run(
        manifest_or_path, trace_loader=load_trace_events,
        sequence_validator=validate_event_sequence,
    )
