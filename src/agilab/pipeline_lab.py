...
_global_app_dispatch_smoke_module = import_agilab_module(
    "agilab.global_pipeline_app_dispatch_smoke",
    current_file=__file__,
    fallback_path=Path(__file__).resolve().parent / "global_pipeline_app_dispatch_smoke.py",
    fallback_name="agilab_global_pipeline_app_dispatch_smoke_fallback",
)
GLOBAL_DAG_QUEUE_UNIT_ID = _global_app_dispatch_smoke_module.QUEUE_UNIT_ID
GLOBAL_DAG_RELAY_UNIT_ID = _global_app_dispatch_smoke_module.RELAY_UNIT_ID
run_global_dag_queue_baseline_app = _global_app_dispatch_smoke_module.run_queue_baseline_app
run_global_dag_relay_followup_app = _global_app_dispatch_smoke_module.run_relay_followup_app
...
class GlobalDagStageExecutionResult:
    ok: bool
    message: str
    state: Dict[str, Any]
...
def _global_dag_real_run_root(lab_dir: Path, unit_id: str) -> Path:
...
def _controlled_global_dag_real_run_supported(
    ...
    if dag_path.resolve(strict=False) != expected:
        return False
    units = {str(unit.get("id", "")): unit for unit in _global_dag_units(state)}
...
def _global_dag_unit(state: Dict[str, Any], unit_id: str) -> dict[str, Any] | None:
...
def _global_dag_events(state: Dict[str, Any]) -> list[dict[str, Any]]:
...
def _global_dag_append_event(
    ...
    *,
    ...
    _global_dag_events(state).append(
        {
            "timestamp": timestamp,
            "kind": kind,
            "unit_id": unit_id,
...
def _global_dag_replace_artifact(state: Dict[str, Any], artifact: dict[str, Any]) -> None:
...
def _global_dag_refresh_summary(state: Dict[str, Any]) -> Dict[str, Any]:
    units = _global_dag_units(state)
    ...
    state["summary"] = {
        ...
        "available_artifact_ids": sorted(_available_artifact_ids(state)),
...
def _global_dag_mark_real_execution(
    state: Dict[str, Any],
    unit: dict[str, Any],
    *,
    result: Dict[str, Any],
...
def _global_dag_unblock_relay_after_queue(state: Dict[str, Any], *, timestamp: str) -> None:
    relay = _global_dag_unit(state, GLOBAL_DAG_RELAY_UNIT_ID)
...
def _global_dag_queue_result_for_relay(state: Dict[str, Any]) -> dict[str, Any]:
    queue = _global_dag_unit(state, GLOBAL_DAG_QUEUE_UNIT_ID)
...
def _global_dag_update_real_execution_provenance(
    state: Dict[str, Any],
    *,
    executed_unit_id: str,
    timestamp: str,
...
def _run_next_controlled_global_dag_stage(
    ...
    *,
    ...
    if not _controlled_global_dag_real_run_supported(mutable_state, dag_path, repo_root):
        return GlobalDagStageExecutionResult(
            ok=False,
            message=(
    ...
    queue = _global_dag_unit(mutable_state, GLOBAL_DAG_QUEUE_UNIT_ID)
    relay = _global_dag_unit(mutable_state, GLOBAL_DAG_RELAY_UNIT_ID)
    if not isinstance(queue, dict) or not isinstance(relay, dict):
        return GlobalDagStageExecutionResult(
            ok=False,
    ...
    if queue_status == "runnable":
        run_root = _global_dag_real_run_root(lab_dir, GLOBAL_DAG_QUEUE_UNIT_ID)
        result = run_queue_fn(repo_root=repo_root, run_root=run_root)
        _global_dag_mark_real_execution(
            mutable_state,
            queue,
            result=dict(result),
            timestamp=timestamp,
        ...
        _global_dag_unblock_relay_after_queue(mutable_state, timestamp=timestamp)
        _global_dag_update_real_execution_provenance(
            mutable_state,
            executed_unit_id=GLOBAL_DAG_QUEUE_UNIT_ID,
            timestamp=timestamp,
        )
        _global_dag_refresh_summary(mutable_state)
        return GlobalDagStageExecutionResult(
            ok=True,
    ...
    if relay_status in {"runnable", "blocked"} and "queue_metrics" in _available_artifact_ids(mutable_state):
        if relay_status == "blocked":
            _global_dag_unblock_relay_after_queue(mutable_state, timestamp=timestamp)
        run_root = _global_dag_real_run_root(lab_dir, GLOBAL_DAG_RELAY_UNIT_ID)
        queue_result = _global_dag_queue_result_for_relay(mutable_state)
        result = run_relay_fn(repo_root=repo_root, run_root=run_root, queue_result=queue_result)
        _global_dag_mark_real_execution(
            mutable_state,
            relay,
            result=dict(result),
    ...
    with st.expander("Multi-app DAG orchestration", expanded=True):
        ...
        if real_run_supported:
            ...
            if run_stage_clicked:
                try:
                    result = _run_next_controlled_global_dag_stage(
                        state,
                        repo_root=repo_root,
                        dag_path=dag_path,
                        lab_dir=lab_dir,
