...
**Runnable app project.** Installed `AGI_install_*` and `AGI_run_*` helpers run `minimal_app_project` as a real AGILAB app project.


## Purpose

Runs `minimal_app_project`, the smallest built-in template for checking that AGILAB
...
```bash
python ~/log/execute/minimal_app/AGI_install_minimal_app.py
```

## Run

```bash
python ~/log/execute/minimal_app/AGI_run_minimal_app.py
```
...
Open `AGI_run_minimal_app.py` and look for these lines first:

- `APP = "minimal_app_project"` selects the minimal built-in app.
