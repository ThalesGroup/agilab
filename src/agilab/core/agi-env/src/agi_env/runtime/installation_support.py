...
def locate_agilab_installation_path(
    *,
    ...
    try:
        base_dir = resolve_package_dir_from_module_file(module_path, "agi_env")
    except FileNotFoundError:
        base_dir = module_path.parents[2] / "agi_env"
    candidate_repo = resolve_agilab_source_root_from_module_file(
        module_path,
        legacy_parent_index=4,
    )
    if candidate_repo is not None and (candidate_repo / "apps").exists():
