...
_label_for_stage_runtime_impl = _pipeline_runtime_support_module.label_for_stage_runtime
...
_python_for_stage_impl = _pipeline_runtime_support_module.python_for_stage
...
_run_locked_stage_impl = _pipeline_runtime_support_module.run_locked_stage
...
MLFLOW_STAGE_RUN_ID_ENV = "AGILAB_PIPELINE_MLFLOW_RUN_ID"
...
def build_mlflow_process_env(
    ...
    *,
    ...
    return _build_mlflow_process_env_impl(
        ...
        stage_run_id_env=MLFLOW_STAGE_RUN_ID_ENV,
        run_id=run_id,
        base_env=base_env,
    )
...
def wrap_code_with_mlflow_resume(code: str) -> str:
    ...
    return _wrap_code_with_mlflow_resume_impl(code, stage_run_id_env=MLFLOW_STAGE_RUN_ID_ENV)
...
def ensure_safe_service_template(
    ...
    *,
    ...
    """Create or update an autogenerated safe service snippet file near lab stages."""
...
def uses_controller_python(engine: str | None, code: str | None) -> bool:
    """Return True when a stage should execute in the current AGILab/controller env."""
    return _uses_controller_python_impl(engine, code)


def python_for_stage(venv_root: str | Path | None, *, engine: str | None, code: str | None) -> Path:
    """Choose the python executable for one stage."""
    return _python_for_stage_impl(venv_root, engine=engine, code=code, sys_executable=sys.executable)


def label_for_stage_runtime(venv_root: str | Path | None, *, engine: str | None, code: str | None) -> str:
    """Return a readable runtime label for the stage log."""
    return _label_for_stage_runtime_impl(venv_root, engine=engine, code=code, sys_executable=sys.executable)
...
def run_locked_stage(
    ...
    *,
    ...
    _run_locked_stage_impl(
        ...
        label_for_stage_runtime_fn=label_for_stage_runtime,
        start_mlflow_run_fn=start_mlflow_run,
        build_mlflow_process_env_fn=build_mlflow_process_env,
        log_mlflow_artifacts_fn=log_mlflow_artifacts,
        run_lab_fn=run_lab,
        python_for_stage_fn=python_for_stage,
