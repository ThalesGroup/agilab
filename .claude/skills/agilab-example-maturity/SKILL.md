...
- It exposes typed or structured inputs, structured outputs, the workflow view,
  the exact run command, and the production boundary before asking users to
  adapt it. A "wow" example is not mature if it only shows a UI trick without a
...
## Pedagogic and Diagnostic Examples

For examples that teach, grade, diagnose, or generate exercise content:

- Make the learner-facing metric explicit in both the UI and artifacts. If the
  page displays a student score, the run summary should persist the same field
  name, for example `student_score`.
- Reducers should aggregate the same metric with a clear name such as
  `student_score_mean`; avoid a second hidden scoring scale in the reducer.
- Generated content should use a validated schema with clear required fields.
  Fail with an actionable message when the local engine returns invalid JSON or
  omits required fields.
- The README should explain whether content is deterministic fixture content,
  generated by a local contract engine, or generated by a real local model.
- Tests should assert that generated CSV/JSON artifacts contain the public fields
  promised by the README, not just that the app completed without raising.

## Inspiration-To-Example Rule

When an external framework or product inspires an example, preserve the useful
learning pattern but keep the example AGILAB-native:

- declarative workflow ideas should map to `lab_stages.toml`,
  `pipeline_view.*`, dry-run validation, or run evidence;
- agent/tool catalog ideas should map to app metadata, package metadata,
  capability manifests, or docs indexes;
- visual playground ideas should still write deterministic evidence or an
  exportable artifact, not only an interactive screen;
- production-grade claims require a clear handoff boundary, expected artifacts,
  local validation command, and residual-risk note.
