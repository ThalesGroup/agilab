...
If the work is primarily about one scientific or technical figure for a report,
slide, README, architecture note, methodology figure, training loop, pipeline,
or comparison panel, prefer `$scientific-svg-figures` first. Use this skill
when the user needs a reusable visual language or a consistent figure family,
not just one strong technical diagram.
...
- For one publication-grade scientific or technical SVG, use `scientific-svg-figures`
...
7. For each diagram family, define the local alignment contracts explicitly:
   - shared bottoms
   - shared centers
   - side-column offsets
   - panel-to-inner-card gutters
8. Record the export and review contract so the next editor does not improvise it.
...
- Decide whether each local complaint should be solved by:
  - a strict constrained move
  - a local sub-ensemble rebalance
  - a family-wide geometry normalization
  Do not improvise that scope mid-edit.
