...
persist_local_llm_selection() {
    ...
    persist_local_llm_env_for_family "$first_family" "$HOME/.agilab/.env"
    persist_local_llm_env_for_family "$first_family" "$HOME/.local/share/agilab/.env"
...
write_tokki_scope() {
    ...
    # install time from repo maps (tokki read/query still reach them).
    local scope_dir="$AGI_INSTALL_PATH/.tokki"
    local scope_file="$scope_dir/scope"
    local manifest="${INSTALLED_APPS_FILE:-$HOME/.local/share/agilab/installed_apps.txt}"
    if [[ ! -f "$manifest" ]]; then
        return 0
    fi
    local -a installed=()
    ...
    local dir name rel selected app
    mkdir -p -- "$scope_dir"
    {
        echo "# generated by agilab install.sh on $(date -u +%Y-%m-%dT%H:%M:%SZ)"
        echo "# apps not selected at install; tokki excludes them from repo maps"
        echo "# excluded apps live in the external apps repository (APPS_REPOSITORY in ~/.local/share/agilab/.env)"
        for dir in "$AGI_INSTALL_PATH"/src/agilab/apps/builtin/*_project "$AGI_INSTALL_PATH"/src/agilab/apps/*_project; do
            [[ -d "$dir" ]] || continue
            name="$(basename -- "$dir")"
            selected=0
            ...
            if (( ! selected )); then
                rel="${dir#"$AGI_INSTALL_PATH"/}"
                echo "exclude $rel/**"
            fi
        done
    } > "$scope_file"
    echo -e "${BLUE}Tokki scope manifest:${NC} $scope_file"
}
