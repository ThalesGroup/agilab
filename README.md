...
their schema, and shows reducer name, partial count, row/source totals, engines,
execution models, and artifact path. The remaining scope is wider adoption
beyond benchmark apps, not the shared reducer interface, the benchmark app
migrations, artifact surfacing, or the public benchmark.
...
| Research experimentation | `4.0 / 5` | Templates, isolated `uv`, `lab_steps.toml`, MLflow-tracked runs, analysis pages, shared `agi_node` reduce contract, surfaced pandas/polars benchmark reduce artifacts, and public reduce benchmark: `0.003s` vs `5.0s`. | Broader non-benchmark app migrations are not complete. |
