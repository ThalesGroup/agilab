...
when present. The remaining scope is wider adoption beyond the migrated public
apps, not the shared reducer interface, migrated artifacts, artifact surfacing,
or the public benchmark.
...
| Research experimentation | `4.0 / 5` | Templates, isolated `uv`, `lab_steps.toml`, MLflow-tracked runs, analysis pages, shared `agi_node` reduce contract, surfaced pandas/polars benchmark, flight, meteo forecast, and UAV queue-family reduce artifacts, and public reduce benchmark: `0.003s` vs `5.0s`. | Broader app migrations beyond the migrated public apps are not complete. |
