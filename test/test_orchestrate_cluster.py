...
def test_canonical_workflow_workers_data_path_is_stable_across_parameter_saves(tmp_path):
    share = tmp_path / "clustershare" / "agi"
    ...
    cluster_params = {
        ...
        "workers_data_path": (
            f"clustershare/agi/workflows/{session_id}"
            f"/agi/workflows/{session_id}/flight_trajectory/dataset"
        ),
    }
    expected = f"clustershare/agi/workflows/{session_id}"

    first_save = orchestrate_cluster.canonical_workflow_workers_data_path(
        cluster_params,
        env,
        project_name="flight_trajectory_project",
    )
    cluster_params["workers_data_path"] = first_save
    second_save = orchestrate_cluster.canonical_workflow_workers_data_path(
        cluster_params,
        env,
        project_name="flight_trajectory_project",
    )

    assert first_save == expected
    assert second_save == expected
    assert first_save.count(session_id) == 1


def test_canonical_workflow_workers_data_path_preserves_operator_overrides(tmp_path):
    env = SimpleNamespace(
        home_abs=tmp_path,
        AGI_CLUSTER_SHARE=str(tmp_path / "clustershare" / "agi"),
        user="agi",
    )
    base_params = {
        "user": "agi",
    ...
    custom_params = {**base_params, "workers_data_path": "/external/shared/data"}
    empty_params = {**base_params, "workers_data_path": ""}

    assert (
        orchestrate_cluster.canonical_workflow_workers_data_path(
            custom_params,
            env,
            project_name="flight_trajectory_project",
        )
