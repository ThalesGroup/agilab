...
APP_FORM = "src/agilab/apps/builtin/meteo_forecast_project/src/app_args_form.py"
...
def test_meteo_forecast_form_renders_and_persists_args(tmp_path: Path) -> None:
    ...
    assert at.text_input(key="meteo_forecast_project:app_args_form:station").value == "Paris-Montsouris"
    assert at.selectbox(key="meteo_forecast_project:app_args_form:target_column").value == "tmax_c"

    at.text_input(key="meteo_forecast_project:app_args_form:station").set_value("Paris-Montsouris")
    at.number_input(key="meteo_forecast_project:app_args_form:lags").set_value(8)
    at.run()
