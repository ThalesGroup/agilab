...
def test_render_project_action_sidebar_switches_to_existing_project_action(monkeypatch) -> None:
    module = _load_module()
    switched: list[Path] = []
    ...
    class Target:
        ...
        @staticmethod
        def button(label, *, key, **_kwargs):
            rendered.append((label, key))
            return label == "Create project"
    ...
    selected = module.render_project_action_sidebar(
        streamlit,
        "alpha_project",
        container=Target(),
    )
    ...
    assert ("Edit files", "project_action__edit") in rendered
    assert ("Create project", "project_action__create") in rendered
    assert streamlit.session_state["sidebar_selection"] == "Create"
    assert streamlit.query_params == {
        "active_app": "alpha_project",
        "sidebar_selection": "Create",
    }
    assert switched == [Path("pages/1_PROJECT.py")]


def test_render_project_action_sidebar_disables_actions_without_project() -> None:
    module = _load_module()
    ...
    streamlit = SimpleNamespace(session_state={}, sidebar=Target(), query_params={})

    assert module.render_project_action_sidebar(streamlit, "", container=Target()) is None
    assert events == [("info", "Select a project to enable project actions.")]
