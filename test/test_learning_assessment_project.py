...
APP_ROOT = Path("src/agilab/apps/builtin/learning_assessment_project").resolve()
...
def test_tescia_workflow_stages_materialize_declared_artifacts(
    ...
    stages = payload["learning_assessment"]
    ...
    reports = json.loads(
        Path("learning_assessment_workflow/diagnostic_reports.json").read_text(
            encoding="utf-8"
        )
    )
    ...
    assert Path("learning_assessment/reports/classroom_learning_tracks.csv").is_file()
...
class _FakeEnv:
    def __init__(self, tmp_path: Path) -> None:
        ...
        self.verbose = 0
        self.target = "learning_assessment"
...
def _load_app_surface_module():
    sys.modules.pop("learning_assessment.app_surface", None)
    return importlib.import_module("learning_assessment.app_surface")
...
def test_learning_assessment_modules_are_classified_or_entrypoints() -> None:
    package_root = APP_SRC / "learning_assessment"
    ...
    for path in direct_modules:
        ...
        assert target_module.startswith("learning_assessment.")
    ...
    legacy_modules = [
        f"learning_assessment.{path.stem}"
        for path in direct_modules
        if path.name != "__init__.py"
    ]
...
def test_tescia_classified_modules_keep_bundled_resource_paths(monkeypatch) -> None:
    ...
    from learning_assessment.domain.classroom import (
        default_case_bank_path,
        default_classroom_payload_path,
    )
    from learning_assessment.domain.curriculum import default_curriculum_path
    from learning_assessment.ui.app_surface import bundled_cases_path
    ...
    assert (
        ...
        == APP_SRC / "learning_assessment" / "curriculum" / "math_program_2026.json"
    )
...
def test_tescia_args_form_renders_scoring_model_as_latex(monkeypatch, tmp_path) -> None:
    ...
    assert ("Classroom intake", False) in fake_streamlit.expanders
...
def test_tescia_ai_generator_uses_fake_gpt_oss_and_validates_schema(
    ...
    from learning_assessment import generate_cases_with_engine
...
def test_tescia_generator_validates_local_gpt_oss_payload(
    ...
    from learning_assessment.generator import generate_case_file
...
def test_tescia_generator_rejects_bad_numeric_scores(monkeypatch) -> None:
    ...
    from learning_assessment.generator import (
        DiagnosticCaseGenerationError,
        validate_generated_cases,
    )
...
def test_tescia_generator_extracts_provider_payloads_and_rejects_bad_responses(
    ...
    from learning_assessment import DiagnosticCaseGenerationError
    from learning_assessment import generator
    ...
    with pytest.raises(DiagnosticCaseGenerationError, match="did not contain text"):
        generator._gpt_oss_text({"output": "bad"})
...
def test_tescia_app_args_reject_invalid_generation_config(monkeypatch) -> None:
    ...
    from learning_assessment import LearningAssessmentArgs, ensure_defaults, merge_args

    with pytest.raises(ValueError, match="files must select JSON"):
        LearningAssessmentArgs(files="*.txt")
    with pytest.raises(ValueError, match="without directories"):
        LearningAssessmentArgs(files="../*.json")
    with pytest.raises(ValueError, match="generated_cases_filename"):
        LearningAssessmentArgs(generated_cases_filename="../bad.json")
    with pytest.raises(ValueError, match="ai_endpoint is required"):
        LearningAssessmentArgs(case_source="standalone_ai", ai_endpoint="")
    with pytest.raises(ValueError, match="ai_model is required"):
        LearningAssessmentArgs(case_source="standalone_ai", ai_model="")
    with pytest.raises(ValueError, match="ai_topic is required"):
        LearningAssessmentArgs(case_source="standalone_ai", ai_topic="")
    args = LearningAssessmentArgs()
    assert ensure_defaults(args) is args
    assert merge_args(args, {"nfile": 2}).nfile == 2
    assert args.submission_inbox == Path("learning_assessment/submissions")
...
def test_tescia_submission_inbox_stays_workflow_scoped_when_global_inbox_exists(
    ...
    from learning_assessment import LearningAssessment, LearningAssessmentArgs
    from learning_assessment.ui.app_surface import classroom_submission_inbox_dir

    physical_root = tmp_path / "share"
    workflow_root = physical_root / "workflows" / "run-001"
    global_cases = physical_root / "learning_assessment" / "cases"
    global_inbox = physical_root / "learning_assessment" / "submissions"
    ...
    env = WorkflowEnv()
    args = LearningAssessmentArgs(
        data_in=Path("learning_assessment/cases"),
        data_out=Path("learning_assessment/reports"),
        submission_inbox=Path("learning_assessment/submissions"),
    )

    app = LearningAssessment(env, args=args)
    ...
    relative_args = LearningAssessmentArgs(
        submission_inbox=Path("learning_assessment/submissions")
    )
    assert (
...
def test_learning_assessment_selects_evidence_backed_fix(monkeypatch) -> None:
    monkeypatch.syspath_prepend(str(APP_SRC))

    from learning_assessment import diagnose_case
...
def test_tescia_sample_cases_have_complete_regression_coverage(monkeypatch) -> None:
    ...
    from learning_assessment import regression_coverage
...
def test_tescia_classroom_metadata_anonymizes_student_ids(monkeypatch) -> None:
    ...
    summary = __import__("learning_assessment").summarize_report(report)
...
def test_tescia_rejects_student_answer_unknown_ids(monkeypatch) -> None:
    ...
    from learning_assessment import validate_case_payload
...
def test_learning_assessment_scores_unsubmitted_and_partial_answers(monkeypatch) -> None:
    ...
    from learning_assessment import diagnostic as diagnostic_module
...
def test_tescia_math_program_2026_curriculum_contract_is_complete(monkeypatch) -> None:
    ...
    with pytest.raises(ValueError, match="Missing 2026 math curriculum coverage"):
        require_complete_math_program_2026_coverage(cases[:2], curriculum)
...
def test_tescia_math_program_2026_rejects_invalid_curriculum_contracts(
    ...
    from learning_assessment.curriculum import (
        CURRICULUM_SCHEMA,
        case_curriculum_ids,
...
def test_tescia_app_surface_cache_and_artifact_edge_paths(
    ...
    class BadPath:
        ...
        def expanduser(self):
            self.calls += 1
            return self if self.calls == 1 else Path("fallback")

        def resolve(self):
            raise RuntimeError("cannot resolve")

    paths_for_bad: list[Path] = []
    module._append_unique_path(paths_for_bad, BadPath())
    assert paths_for_bad == [Path("fallback")]
    ...
    active_app = tmp_path / "learning_assessment_project"
    active_app.mkdir()
    ...
    monkeypatch.setattr(
        ...
        lambda _path: (
            env,
            SimpleNamespace(submission_inbox=Path("learning_assessment/submissions")),
        ),
    )
    assert module.classroom_submission_inbox_dir(runtime_app) == (
        env.share_root_path() / "learning_assessment" / "submissions"
    )
...
def test_tescia_app_surface_runtime_loader_and_filter_edges(
    ...
    created_env = FakeAgiEnv(
        apps_path=tmp_path, app="learning_assessment_project", verbose=0
    )
    monkeypatch.setitem(
        sys.modules, "agi_env", SimpleNamespace(AgiEnv=lambda **_kwargs: created_env)
    )

    import learning_assessment.app_args as app_args

    args = app_args.LearningAssessmentArgs(data_out=Path("custom_reports"))
    ...
    env, loaded_args = module._load_runtime_env_and_args(
        tmp_path / "learning_assessment_project"
    )
...
def test_tescia_app_surface_render_covers_classroom_upload_and_partial_status(
    ...
    assert module.classroom_submission_inbox_dir(active_app=active_app) == (
        fake_env.share_root_path() / "learning_assessment" / "submissions"
    )
...
def test_tescia_app_surface_error_refresh_last_run_authoring_and_warning(
    ...
    class BranchStreamlit(_FakeStreamlit):
        def __init__(self, env: _FakeEnv) -> None:
            ...
            cache_data.clear = clear
            self.cache_data = cache_data

        def button(self, label, *_args, **_kwargs) -> bool:
            self.buttons.append(label)
            return label in {"Evaluate answer", "Refresh classroom artifacts"}
        ...
        def text_input(self, label="", *_args, **kwargs):
            values = {
                "Case id": "teacher_case",
                "Title": "Teacher title",
                "Reference fix id": "teacher_fix",
            }
            return values.get(label, kwargs.get("value", ""))

        def text_area(self, label="", *_args, **kwargs):
            values = {
                "Student prompt": "Solve this.",
                "Reference root cause": "Use the rubric.",
            }
            return values.get(label, kwargs.get("value", ""))

        def multiselect(self, _label, options, *_args, **_kwargs):
            return options[:1]
    ...
    called = []
    monkeypatch.setattr(module, "render", lambda **kwargs: called.append(kwargs))
    module.main()
...
def test_tescia_printable_exports_handle_missing_optional_sections(
    ...
    assert (
        ...
        .startswith("# Learning & Assessment Correction Sheets")
    )
...
def test_tescia_classroom_batch_scores_and_exports_teacher_artifacts(
    ...
    from learning_assessment import diagnose_case
    from learning_assessment import classroom as classroom_module
    ...
    assert markdown.startswith("# Learning & Assessment Classroom Teacher Summary")
    ...
    partial_paths = write_classroom_partial_artifacts(
        rebuilt,
        tmp_path / "live_classroom",
        worker_id=7,
        source_file="classroom_submissions.json",
    )
...
def test_tescia_reduce_contract_rejects_missing_or_empty_payloads(
    ...
    from learning_assessment import (
        LEARNING_ASSESSMENT_REDUCE_CONTRACT,
        partial_from_diagnostic_summary,
    ...
    with pytest.raises(ValueError, match="requires at least one partial"):
        LEARNING_ASSESSMENT_REDUCE_CONTRACT.build_artifact([])
...
def test_tescia_manager_seeds_sample_and_builds_distribution(
    ...
    assert (env.share_root_path() / "learning_assessment" / "submissions").is_dir()
    ...
    app.args.include_submission_inbox = False
...
def test_tescia_manager_handles_config_and_dataset_edge_paths(
    ...
    with pytest.raises(ValueError, match="Invalid TeSciA diagnostic arguments"):
        LearningAssessment(env, files="*.txt")

    app = LearningAssessment(env, reset_target=True)
    ...
    loaded = LearningAssessment.from_toml(env, settings)
    assert loaded.as_dict()["case_source"] == "bundled"
    dump_args(LearningAssessmentArgs(nfile=3), settings)
    ...
    with pytest.raises(
        ...
        LearningAssessment(missing_env)
    ...
    with pytest.raises(
        ...
        LearningAssessment(missing_classroom_env, case_source="bundled_classroom")
...
def test_tescia_manager_distribution_multiple_files_and_empty_pattern(
    ...
    app = LearningAssessment(env, data_in=Path("custom_cases"), nfile=2)
...
def test_tescia_manager_can_seed_generated_cases_from_injected_ai_engine(
    ...
    seeded = env.share_root_path() / "learning_assessment" / "cases" / "ai_cases.json"
    assert seeded.is_file()
...
def test_tescia_manager_can_seed_bundled_classroom_sample(
    ...
    env = _FakeEnv(tmp_path)
    app = LearningAssessment(env, case_source="bundled_classroom")
    ...
    assert (label_key, size_key, size_unit) == ("diagnostic_file", "size_kb", "KB")
    assert (
        LearningAssessment(env, case_source="bundled_classroom").build_distribution(
            {"127.0.0.1": 1}
        )[0]
        == work_plan
    )
...
def test_tescia_manager_reuses_existing_generated_cases_and_zero_nfile_branch(
    ...
    app = LearningAssessment(
        env,
        data_in=Path("generated_cases"),
        case_source="standalone_ai",
        generated_cases_filename="existing.json",
        _generate_case_file_fn=unexpected_generate,
    )
    app.args.nfile = 0
...
def test_tescia_app_args_form_exposes_standalone_ai_controls(
    ...
    source = at.selectbox(key="learning_assessment_project:app_args_form:case_source")
    ...
    assert not at.exception
    assert (
        at.selectbox(key="learning_assessment_project:app_args_form:case_source").value
        == "bundled_classroom"
    )
    assert not any(text_input.label == "AI endpoint" for text_input in at.text_input)
    ...
    assert not at.exception
    assert (
        at.selectbox(key="learning_assessment_project:app_args_form:ai_provider").value
        == "gpt-oss"
    )
    assert (
        at.text_input(key="learning_assessment_project:app_args_form:ai_endpoint").value
        == DEFAULT_GPT_OSS_ENDPOINT
    )
    assert (
        at.text_input(key="learning_assessment_project:app_args_form:ai_model").value
...
def test_tescia_worker_exports_json_csv_and_reduce_artifacts(
    ...
    env = _FakeEnv(tmp_path)
    input_dir = env.share_root_path() / "learning_assessment" / "cases"
    input_dir.mkdir(parents=True, exist_ok=True)
    case_file = input_dir / "cases.json"
    case_file.write_text(SAMPLE_CASES.read_text(encoding="utf-8"), encoding="utf-8")

    worker = LearningAssessmentWorker()
    worker.env = env
    worker.args = SimpleNamespace(
        data_in=input_dir,
        data_out=env.share_root_path() / "learning_assessment" / "reports",
    ...
    preexisting_root = Path(worker.data_out) / "cluster_share_sshfs"
    preexisting_root.mkdir(parents=True)
    stale_file = preexisting_root / "stale.txt"
    stale_file.write_text("stale", encoding="utf-8")
    worker.work_done(df)
    assert not stale_file.exists()

    output_root = Path(worker.data_out)
    assert (output_root / "learning_assessment_summary.csv").is_file()
    summary_payload = (output_root / "learning_assessment_summary.csv").read_text(
        encoding="utf-8"
    )
    ...
    export_root = env.AGILAB_EXPORT_ABS / env.target / "learning_assessment"
    assert (
        export_root
        / "pipeline_dag_stale_preview"
...
def test_tescia_worker_accepts_classroom_submission_batches(
    ...
    assert partial_report["partial"]["source_label"] == "classroom_submissions"
...
def test_tescia_worker_helper_and_empty_paths(monkeypatch, tmp_path) -> None:
    ...
    import pandas as pd
    from learning_assessment_worker.learning_assessment_worker import (
        LearningAssessmentWorker,
        _artifact_dir,
        _sanitize_slug,
    )
    ...
    worker._write_artifact_bundle(tmp_path / "empty_bundle", pd.DataFrame())
    ...
    object_worker = LearningAssessmentWorker()
...
def test_tescia_app_surface_handles_classroom_inbox_and_partial_reports(
    ...
    args_model = SimpleNamespace(
        data_out=tmp_path / "reports",
        submission_inbox=Path("learning_assessment/submissions"),
    )
    inbox_dir = module.classroom_submission_inbox_dir(env=env, args_model=args_model)
    assert inbox_dir == env.share_root_path() / "learning_assessment" / "submissions"

    upload = SimpleNamespace(
        name="Period A submissions.json",
        getvalue=lambda: SAMPLE_CLASSROOM.read_bytes(),
    )
    saved = module.save_classroom_uploads([upload], inbox_dir)
    assert saved == [inbox_dir / "Period_A_submissions.json"]
    saved_payload = json.loads(saved[0].read_text(encoding="utf-8"))
    assert saved_payload["schema"] == "agilab.tescia_diagnostic.classroom.v1"

    partial_root = tmp_path / "reports" / "classroom" / "partials"
    partial_root.mkdir(parents=True)
    partial_report = module.classroom_preview_report(module.load_cases())
...
def test_tescia_app_surface_resolves_active_app_from_argv(
    ...
    monkeypatch.syspath_prepend(str(APP_SRC))
    module = _load_app_surface_module()
    app_path = tmp_path / "learning_assessment_project"
    app_path.mkdir()
