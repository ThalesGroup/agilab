...
| apps | app install (local) | $ProjectFileDir$/src/agilab/apps/install.py | $Prompt:selected app:$ProjectFileDir$/src/agilab/apps/builtin/flight_project$ --install-type "1" --verbose 1 | $ProjectFileDir$ | PYTHONUNBUFFERED=1;UV_NO_SYNC=1 | cd $ProjectFileDir$ && uv run python $ProjectFileDir$/src/agilab/apps/install.py $Prompt:selected app:$ProjectFileDir$/src/agilab/apps/builtin/flight_project$ --install-type "1" --verbose 1 | uv (agi-cluster) |
