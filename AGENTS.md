...
| agilab | testpypi publish | $ProjectFileDir$/tools/pypi_publish.py | --repo testpypi --leave-most-recent --verbose --cleanup $Prompt:Cleanup credentials$ | $ProjectFileDir$ | PYTHONUNBUFFERED=1 PYDEVD_USE_FRAME_EVAL=NO;UV_NO_SYNC=1 | cd $ProjectFileDir$ && uv run python $ProjectFileDir$/tools/pypi_publish.py --repo testpypi --leave-most-recent --verbose --cleanup $Prompt:Cleanup credentials$ | uv (agilab) |
