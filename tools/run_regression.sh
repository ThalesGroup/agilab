...
if [[ "$RUN_APPS" != "0" ]]; then
  ...
  while IFS= read -r app_dir; do
    ...
    # flight_legacy relies on a worker that may not exist locally; skip to avoid install failures
    if [[ "$app_name" == "flight_legacy" ]]; then
      echo "Skipping $app_dir (flight_legacy worker not provisioned)."
      continue
    fi
