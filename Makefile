...
# Path to the private thales_agilab repo (override on invocation if needed)
THALES_DIR ?= ../thales_agilab
...
docs-help:
	@echo "Docs targets:"
	@echo "  make docs-build    # Build Sphinx site in thales_agilab (uses uv group=sphinx)"
	...
	@echo ""
	@echo "Overrides: make docs-all THALES_DIR=/path/to/thales_agilab"
...
docs-publish:
	...
	@GIT_AUTHOR_DATE="$$(date -u +"%Y-%m-%dT%H:%M:%SZ")" GIT_COMMITTER_DATE="$$(date -u +"%Y-%m-%dT%H:%M:%SZ")" \
		git -C "$(AGILAB_DIR)" commit -m "docs: sync Sphinx site from thales_agilab" || true
